...
parser: ArgumentParser = ArgumentParser(description='Bacteria family identification tool.', add_help=True)
parser.add_argument("-l", "--locals", help="Display locals on error.", action='store_true')
...
parser_database: ArgumentParser = subparsers.add_parser('build',
    help="Creates the database from the specified set of files.")

parser_database.add_argument("-p", "--parameters",
    help="Specifies a parameter file",
    type=str,
    default=f'{path.dirname(__file__)}/parameters_files/params.json'
)

parser_database.add_argument("database_name",
    help="Name for database",
    type=str
)

parser_database.add_argument("input_folder",
    help="Input folder contains reference genomes",
    type=str
)
...
parser_prediction: ArgumentParser = subparsers.add_parser('predict',
    help="Creates the samples and evaluates them.")

parser_prediction.add_argument("-p", "--parameters",
    help="Specifies a parameter file",
    type=str,
    default=f'{path.dirname(__file__)}/parameters_files/params.json'
)

parser_prediction.add_argument("database_name",
    help="Name for database",
    type=str
)

parser_prediction.add_argument("input_folder",
    help="Input folder containig unknown genomes",
    type=str
)

parser_prediction.add_argument("output_folder",
...
def main() -> None:
    ...
    if args.subcommands == 'build':
        ...
        print(
            f"[dark_orange]Database sucessfully built @ {output_path} in {database_time} min"
        )
        ...
        print(f"[dark_orange]Finished computing models, tree @ {phylo_path} in {model_time} min : TOTAL {database_time+ model_time} min")
    ...
    if args.subcommands == 'predict':
        ...
        model_dir = f"{path.dirname(__file__)}/model/{args.database_name}"
        try:
        ...
        for genome in tqdm(input_files[:50]):
            ...
            pargs = [(id_sequence, dna_sequence, params, tree, threshold, args.parameters, model_dir)
            ...
            makedirs(f"{path.dirname(report_path)}", exist_ok=True)
            
            with open(report_path, 'w', encoding='utf-8') as jwriter:
                dump({identifier: prediction_results[i] for i, (identifier, _, _, _, _, _,_) in enumerate(
                    pargs)}, jwriter)
